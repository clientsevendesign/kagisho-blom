...
const Navbar = ({ theme, setTheme, player, isAuthenticated, onLogout }) => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 backdrop-blur-xl border-b px-6 py-4 flex justify-between items-center ${
      ...
      <Link to="/" className={`font-black text-2xl tracking-tighter uppercase ${textColor}`}>
        {initials}<span className="text-soccer-red text-sm italic">{player?.jersey_number || '15'}</span>
      </Link>
      ...
      <div className={`hidden md:flex gap-8 items-center text-[10px] font-bold uppercase tracking-widest ${textColor}`}>
        ...
        {isAuthenticated ? (
          <button
            onClick={onLogout}
            className="flex items-center gap-2 px-4 py-2 rounded-full border border-current opacity-60 hover:opacity-100 hover:text-soccer-red hover:border-soccer-red transition"
          >
            <LogOut size={10} /> <span>Logout</span>
          </button>
        ) : (
          <Link
            to="/login"
            className={`flex items-center gap-2 px-4 py-2 rounded-full border transition ${
              location.pathname === '/login'
                ? 'bg-soccer-red text-white border-soccer-red'
