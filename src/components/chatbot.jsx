...
import { X, Send, MessageCircle, Phone, Mail, Bot, Download } from 'lucide-react';
...
const Chatbot = ({ accentColor, theme, player }) => {
    ...
    const sendMessage = async (text) => {
        ...
        // Build clean history for API — MUST start with a user message for Gemini
        // Filter out the initial greeting (assistant) so first message is always user
        ...
        try {
            ...
            if (err.response?.status === 503 || errMsg.includes('GEMINI')) {
                setMessages(prev => [...prev, {
                    role: 'assistant',
                    type: 'contact',
                    content: "My AI assistant isn't configured yet, but you can reach my team directly:",
                ...
                setMessages(prev => [...prev, {
                    role: 'assistant',
                    content: "Sorry, I'm having a moment. Try again shortly!",
                }]);
            }
        } finally {
            setLoading(false);
            ...
            <AnimatePresence>
                {open && (
                    ...
                    >
                        ...
                        <div className={`flex items-center gap-3 px-5 py-4 border-b ${headerBg} ${borderCol}`}>
                            <div className="relative">
                                ...
                                <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 rounded-full border-2"
                                    style={{ borderColor: isDark ? '#111' : '#fff' }} />
                            </div>
                            <div>
                                <p className={`font-black text-sm ${isDark ? 'text-white' : 'text-neutral-900'}`}>
                                    {player?.name || 'Kagisho Blom'}
                                </p>
                                <p className="text-[10px] font-bold" style={{ color: ac }}>
                                    {player?.position} · {player?.club}
                                </p>
                            </div>
                            <button onClick={() => setOpen(false)}
                                className={`ml-auto p-2 rounded-xl transition ${isDark ? 'text-white/30 hover:text-white hover:bg-white/5' : 'text-neutral-400 hover:text-neutral-900 hover:bg-black/5'}`}>
                                <X size={16} />
                            </button>
                        </div>
                        ...
                        {/* Starter suggestions — only when only greeting exists */}
                        {messages.length === 1 && !loading && (
                            <div className={`px-4 pb-3 flex flex-wrap gap-1.5 border-t pt-3 ${borderCol}`}>
                                {STARTER_SUGGESTIONS.map((s, i) => (
                                    <button key={i} onClick={() => sendMessage(s)}
                                        className={`text-[10px] font-bold px-3 py-1.5 rounded-full border transition hover:opacity-80 ${isDark ? 'border-white/10 text-white/50 hover:border-white/20' : 'border-black/10 text-neutral-500'
                                            }`}>
                                        {s}
                                    </button>
                                ))}
                        ...
                        <div className={`px-4 py-3 border-t ${borderCol}`}>
                            <div className="flex gap-2 items-end">
                                ...
                                <button
                                    ...
                                    className="w-10 h-10 rounded-2xl text-white flex items-center justify-center shrink-0 disabled:opacity-40 hover:brightness-110 transition"
                                    style={{ backgroundColor: ac }}>
                                    <Send size={15} />
                                </button>
                            </div>
                            <p className={`text-[9px] text-center mt-2 ${isDark ? 'text-white/15' : 'text-neutral-300'}`}>
